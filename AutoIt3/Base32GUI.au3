...
; Creates window
GUICreate('Base32 Encoder - ALBANESE Research Lab ' & Chr(169) & ' 2016', 590, 400)
...
While 1
	Switch GUIGetMsg()
		...
		Case $idEncodeButton
			...
			$dEncoded = _Base32_Encode(GUICtrlRead($idEditText))     ; Encode the text with the new codeographic key.
		...
		Case $idDecodeButton
			...
			; Calls the encodeion. Sets the data of editbox with the encodeed string
			$dDecoded = _Base32_Decode(StringReplace(GUICtrlRead($idEditText), @CRLF, ""))     ; Decode the data using the generic password string. The return value is a binary string.
